...
// 
// 0 : methode naive 
// 1 : decomposition avec yaroslasky
// 2 : decomposition avec szeliski
// 3 : mipmap
// 4 : ripmap
...
//H[0][0]=1.899628; H[0][1]=0.554891; H[0][2]=-328.480718;
//H[1][0]=0.374255; H[1][1]=1.899628; H[1][2]=-241.955934;
//H[2][0]=0.000732; H[2][1]=0.001086; H[2][2]=0.787031;
//code impression
...
//fin code impression
	...
	/*H[0][0]=0.5; //j'essaie d'avoir ad-bc=0 pour TH, mais il faut s'assurer t0=0 c'est auch
	H[0][1]=0.5;
	H[0][2]=-18.339451;
	H[1][0]=0.5;
	H[1][1]=0.5;
...
//Methode geo yaro ou szeliski
if(e->interpolation_order==1 || e->interpolation_order==2){

	float *img = malloc(3*(f->w)*(f->h)*sizeof(float));
	float *img_f = malloc(3*(f->w)*(f->h)*sizeof(float));
	
	if(e->pd==3){
        apply_homo_final(e->img,img_f,w,h,f->w,f->h,H,e->interpolation_order);
        ...
        apply_homo_final(img3,img_f,w,h,f->w,f->h,H,e->interpolation_order);
	}
	for(int i=0;i<3*(f->w)*(f->h);i++){(f->rgb)[i]=float_to_byte(img_f[i]);}
	}
	
//methode mipmap
if(e->interpolation_order==3){

    float *mipmap = malloc(sizeof(float)*w*h*6+3);
    for(int g = 0;g<w*h*6;g++){mipmap[g]=0;}
    build_mipmap(e->img,mipmap,w,h,e->pd);
    
	double D[17];
	precal_D_mipmap(H,D,3); //le trois peu varier !
	...
	for (int i = 0; i < f->w; i++)
	{
		...
		{
			int idx = l + 3 * (f->w * j + i);
			double dd = cal_D_mipmap(i,j,D,3);
			float v = mip_mapping_interpolator_at(mipmap, w, h, 3, p[0], p[1], l, dd, 3);
			f->rgb[idx] = float_to_byte(v);
		}
	}
	...
	for(int i=0;i<f->w;i++){
		for(int j=0;j<f->h;j++){
			...
			if(p[0]<0 || p[0]>w || p[1]<0 || p[1]>h ){
				for(int l=0;l<3;l++){f->rgb[(j*f->w+i)*3+l]=0;}
			}
		}
	}
...
//on va construire le ripmap de img.
	...
	build_ripmap(e->img,ripmap,w,e->ih,e->pd,logw);
