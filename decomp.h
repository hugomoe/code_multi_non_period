...
void apply_homo_final(float *img,float *img_f,int w,int h,int w_f,int h_f,double H[3][3],int opt){
...
printf("\ntransformation de l'image en cours\n");
        ...
        float *img2 = malloc(3*w2*h2*sizeof(float));
		if(opt==1){apply_affinite(img1,img2,w1,h1,w2,h2,A);}
		if(opt==2){yaroslavsky(img1,img2,w1,h1,w2,h2,A);}
		//free(img1);
...
printf("application de H0 : done\n");
		...
		if(opt==1){apply_affinite(img3,img4,w3,h3,w4,h4,B);}
		if(opt==2){yaroslavsky(img3,img4,w3,h3,w4,h4,B);}
		//free(img3);
